...
<div style="overflow: auto" class="grzxbj">
    <div class="selfinfo center">
        <div class="rtcont fr">
            <div class="ddzxbt" style="display: inline;font-size: 22px;">我的上架</div>
            <div class="ddzxbt" style="display: inline;"><button id="shangjia"  onclick="uploadwupin()">上架物品</button></div>
            <c:if test="${requestScope.sizezxc==0}">
                <div style="margin-left: 320px;margin-top: 150px ;font-family: 'Microsoft YaHei UI';font-size: 20px">还没有上架任何物品哦亲!</div>
                <br/>
            </c:if>
            <c:forEach items="${requestScope.mySaleList}" var="list">
                <div class="ddxq">
                    <div class="ddspt fl"><img src="<%=request.getContextPath()%>/image/${list.image}" height="80px" width="80px"></div>
                    <div class="ddbh fl">物品名称:${list.title}</div>
                    <div class="ztxx fr">
                        <ul>
                            <li>${list.type}</li>

                            <li><button  class="confirmDescConfirmIcon" id="${list.id}" style="border-radius: 10px;margin-top: 12px;background:rgba(236,216,255,0.53)">物品详情</button></li>
                        </ul>
                    </div>
                    <div class="clear"></div>
                </div>
...
<script>
    function uploadwupin(){
        layer.open({
            ...
            content: "<%=request.getContextPath()%>/frontpage/shangjiawupin.jsp",
        ...
        $.ajax({
            ...
            success:function (JsonObject) {
                ...
                layer.open({
                    ...
                    maxmin: true, //开启最大化最小化按钮
                    content: "<%=request.getContextPath()%>/frontpage/shangjiaDetail.jsp",
                    success: function (layero, index) {
                        // 获取子页面的iframe
                        var iframe = window['layui-layer-iframe' + index];
                        // 向子页面的全局函数child传参
